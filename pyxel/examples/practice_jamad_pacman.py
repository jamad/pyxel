...
class Pacman:#pacman自体を生成するクラス
    def __init__(self):
        self.P=[8,8]# unit: pixel
        self.V=[0,0]# パックマンの進む方向の情報 unit: タイル
        self.T=[1,1]# 今いるタイルを座標化する unit: タイル
...
class App:
    def __init__(self):
        ...
        self.tilemap_state=tilemap(0)#################  抜けてた！ 超重要！！！　偶然でも、よく見つけたな俺
    ...
    def update(self):# ゲーム内情報更新、キー入力処理
        ...
        # clear tile 
        current_tile=self.tilemap_state.get(self.pacman.T[0],self.pacman.T[1])
        if current_tile==64:self.pacman.score += 10# ノーマルクッキーに触れたら
        if current_tile==65:self.pacman.score += 50# パワークッキーに触れたら
        pyxel.tilemap(0).set(self.pacman.T[0],self.pacman.T[1],5)# パックマン後は、何もないブロックに変える
    ...
    def draw(self):# ゲーム内描画処理
        ...
        #dscore描画
        score = "SCORE : {:05}".format(self.pacman.score)
        pyxel.text(1, 132, score, 6)
App()