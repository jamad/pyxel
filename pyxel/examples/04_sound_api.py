...
class App:
    def __init__(self):
        pyxel.init(200, 150, caption="Pixel Sound API",scale=2)
    ...
    def play_music(self, ch0, ch1, ch2):
        if ch0: pyxel.play(0,[0, 1], loop=1)
        else: pyxel.stop(0)
        if ch1: pyxel.play(1,[2, 3], loop=1)
        else: pyxel.stop(1)
        if ch2: pyxel.play(2,4, loop=1)
        else: pyxel.stop(2)
    def update(self):
        if pyxel.btnp(pyxel.KEY_Q): pyxel.quit()
        if pyxel.btnp(pyxel.KEY_1): self.play_music(1, 1, 1)
        if pyxel.btnp(pyxel.KEY_2): self.play_music(1, 0, 0)
        if pyxel.btnp(pyxel.KEY_3): self.play_music(0, 1, 0)
        if pyxel.btnp(pyxel.KEY_4): self.play_music(0, 0, 1)
        if pyxel.btnp(pyxel.KEY_5): self.play_music(0, 0, 0)
    def draw(self):
        pyxel.cls(1)
        ...
        pyxel.text(6, 62, "play(ch,snd,loop=0)", 7)
        pyxel.text(6, 71, "playm(msc,loop=0)", 7)
