...
class TCPClient:
    ...
    def __init__(self, server_addr: AddrType) -> None:
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        ...
        print("Connected to {}:{}, self {}".format(server_addr[0],
                                                   server_addr[1],
                                                   self.sock.getsockname()))

    def register_player(self, index: int, player: Player) -> None:
    ...
    def receive_message(self) -> NetMessage:
        """ Read one server message from socket """
