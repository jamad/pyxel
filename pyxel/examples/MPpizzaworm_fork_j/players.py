...
class Player(ABC):
    ...
    def bind_snake(self, snake_id: int):
...
class Human(Player):
    ...
    def __init__(self, name: str, input_mapper: InputHandler, keyboard_controls: Tuple[int, int]):
        super().__init__(name)
    ...
    def send_update(self, snake_id: int, added_parts: List[SnakePart],num_removed_parts: int):
...
class SimpleAI(Player):# """ Simple AI to test interfaces """
    ...
    def act(self):
        self.snake_input = random.randrange(-5, 6)# """ Generate input """
